...
# သတိပေးချက်တွေကို မှတ်ထားဖို့ (Bot Restart ချရင်တော့ Reset ဖြစ်ပါမယ်)
warns = {}
...
async def auto_mute_handler(client: Client, message: Message):
    # Admin တွေပို့ရင် ဘာမှမလုပ်ဘူး
    member = await message.chat.get_member(message.from_user.id)
    if member.status in ["administrator", "creator"]:
        return

    # စာသားထဲမှာ Link ပါသလား (သို့မဟုတ်) Forward လုပ်ထားသလား စစ်မယ်
    ...
    if has_link or is_forwarded:
        ...
        if current_warns < 3:
            ...
            # သတိပေးစာကို ၁၀ စက္ကန့်နေရင် ပြန်ဖျက်မယ်
            await asyncio.sleep(10)
            await warn_msg.delete()

        else:
            # --- ၃ ကြိမ်ပြည့်လို့ Mute မယ် ---
            try:
